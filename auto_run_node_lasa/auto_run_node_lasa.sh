...
run_both_selectors () {
  ...
  echo "    [DTW]   -> ${outdir}"
  python -m src.experiments.run_experiments_lasa "$@"

  echo "    [LEAST] -> ${outdir}_le"
  python -m src.experiments.run_experiments_lasa "$@" --selector least_effort --out "${outdir}_le"
}
...
for SH in "${SHAPES[@]}"; do
  ...
  CFG="src/train/train_configs/configs_lasa/${SH}/node_train.yaml"
  OUTDIR="auto_run_node_lasa/outputs_newdist_high_freq_9/expts/${SH}"
  mkdir -p "${OUTDIR}"

  echo "[TRAIN] ${SH}"
  python -m src.train.train_node_lasa --config "${CFG}"
