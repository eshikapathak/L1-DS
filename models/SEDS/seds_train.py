...
from models.SEDS.lasa_data import load_shape, get_shape_names, resample
...
def main(args):
    ...
    try:
        # load_shape now returns duration as well
        lasa_shape = load_shape(args.shape)
    except Exception as e:
        print(f"Error loading shape '{args.shape}': {e}")
        available_shapes = get_shape_names()
        if available_shapes:
            print("Available shapes are:", ", ".join(available_shapes))
        sys.exit(1)


    # Original Data dimensions and duration
    n_demos_orig, n_samples_orig, d_orig = lasa_shape.pos.shape
    original_duration = lasa_shape.duration
    print(f"Original data: {n_demos_orig} demos, {n_samples_orig} samples, {d_orig} dimensions, Duration: {original_duration:.3f}s")

    # --- 1b. Optional Resampling (for Training Data) ---
    if args.nsamples_train is not None and args.nsamples_train > 0:
        print(f"Resampling demonstrations to {args.nsamples_train} points for training...")
        try:
            pos_data, vel_data, _ = resample(lasa_shape, nsamples=args.nsamples_train)
            print(f"Resampled training data: {pos_data.shape[0]} demos, {pos_data.shape[1]} samples, {pos_data.shape[2]} dimensions")
        except Exception as e:
            print(f"Error during resampling: {e}. Proceeding with original data for training.")
            pos_data = lasa_shape.pos
            vel_data = lasa_shape.vel
    else:
        # Use original data points for training if no resampling requested
        pos_data = lasa_shape.pos
        vel_data = lasa_shape.vel
        print("Using original data samples for training.")


    # --- 1c. Center Data at Target ---
    target = lasa_shape.pos[0, -1, :] # Use original target for consistency
    print(f"Centering training data around target: {target}")
    ...
    n_demos, n_samples_train_final, d = demos_x.shape # d is n_dim
    ...
    success = seds.train(demos_x, demos_dx) # Train on (potentially resampled) centered data
    ...
    # --- 3. Simulate and Plot ---
    # Calculate n_steps for simulation based on original duration and desired dt
    sim_dt = args.dt
    # Ensure duration is positive, use 1.0 as fallback
    sim_duration = original_duration if original_duration > 1e-6 else 1.0
    # Calculate steps needed: duration = n_steps * dt => n_steps = duration / dt
    # Use math.ceil to ensure we cover the full duration
    n_steps_required = math.ceil(sim_duration / sim_dt)

    print(f"Simulating trajectories using dt={sim_dt:.4f} for original duration {sim_duration:.3f}s ({n_steps_required} steps)...")
    sim_trajs = []
    # Use the *original* (but centered) starting points for simulation consistency
    original_starts_centered = lasa_shape.pos[:, 0, :] - target

    for i in range(n_demos_orig): # Iterate through original number of demos
        x_start = original_starts_centered[i] # Starting point of demo i (in model coords)
        # Simulate using calculated n_steps
        traj, _ = seds.simulate(x_start, dt=sim_dt, n_steps=n_steps_required)
    ...
    title_suffix = f"(Train: {'Resampled ' + str(args.nsamples_train) + 'pts' if args.nsamples_train else 'Original Samples'})"
    fig.suptitle(f'SEDS Training for "{args.shape}" with {args.k} Gaussians {title_suffix}', fontsize=16)

    # Plot 1: Original Demonstrations (Centered)
    ax1 = axes[0]
    ax1.set_title("Training Demonstrations (Centered)")
    # Plot the data actually used for training (demos_x)
    ...
    ax2 = axes[1]
    ax2.set_title(f"SEDS Reproductions (dt={sim_dt:.4f}, {n_steps_required} steps)")
    for i, traj in enumerate(sim_trajs):
        ax2.plot(traj[:, 0], traj[:, 1], 'g-', alpha=0.8, label="SEDS Simulation" if i == 0 else None)
        # Plot start points used for simulation
        ax2.scatter(original_starts_centered[i, 0], original_starts_centered[i, 1], c='g', marker='o')
    ...
    # Create grid based on Plot 1 limits (which shows training data)
    xlim = ax1.get_xlim()
    ylim = ax1.get_ylim()
    x_range = xlim[1] - xlim[0]; y_range = ylim[1] - ylim[0]
    padding = 0.15 * max(x_range, y_range) if max(x_range, y_range) > 1e-6 else 0.5
    ...
    try:
        ...
        DX_grid = DX.reshape(X.shape[0], X.shape[1], d)

        speed = np.sqrt(DX_grid[:, :, 0]**2 + DX_grid[:, :, 1]**2)
        # Normalize line width safely
        max_speed = speed.max()
        lw = 1.5 * speed / max_speed if max_speed > 1e-6 else np.ones_like(speed) * 0.5
    ...
    # Overlay training demos (demos_x) for context
    for demo in demos_x:
        ax3.plot(demo[:, 0], demo[:, 1], 'b-', alpha=0.2)
    ...
    if args.outdir:
        try:
            ...
            resample_tag = f"_trainResamp{args.nsamples_train}" if args.nsamples_train else ""
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Warning: Could not get shape names for help text: {e}")
        shape_names = []
        shape_help = "Name of the LASA shape (e.g., 'Worm')."

    parser.add_argument("--shape", type=str, default="Worm", help=shape_help)
    parser.add_argument("-k", type=int, default=6, help="Number of Gaussian components (K). Default: 6")
    parser.add_argument("--nsamples_train", type=int, default=None, help="Optional: Resample demonstrations to this many points *before training*. Default: Use original samples.")

    # Simulation params for plotting reproductions
    parser.add_argument("--dt", type=float, default=0.01, help="Simulation time step for generating reproductions. Default: 0.01")
    # REMOVED n_steps argument - it's now calculated based on original duration
    # parser.add_argument("--n_steps", type=int, default=500, help="Number of simulation steps for generating reproductions.")
    ...
    # Final shape name validation
    # Re-fetch names in case initial attempt failed
    current_shape_names = get_shape_names()
    if parsed_args.shape not in current_shape_names and len(current_shape_names) > 0:
        print(f"\n--- ERROR: Shape '{parsed_args.shape}' not found ---", file=sys.stderr)
        print("Available shapes are:", file=sys.stderr)
        print(", ".join(current_shape_names), file=sys.stderr)
        print("------------------------------------------", file=sys.stderr)
        sys.exit(1)
    elif not current_shape_names:
         print(f"Warning: Could not validate shape name '{parsed_args.shape}' as no shapes were found.")
