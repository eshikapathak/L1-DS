...
def main():
    ...
    # Concentrate multisine components around ~10 Hz
    ...
    titles = [
        ...
        "Unmatched constant",
        "Unmatched multisine",
        "Matched multisine + Unmatched pulses",
    ]
    ...
    for idx, (ax, tt, vec, title) in enumerate(zip(axes, time_axes, series, titles), start=1):
        ...
        if idx == 1:
            ...
            ax.legend([line1, line2], [r"$d(p_1)$", r"$d(p_2)$"],
                      loc="upper right", frameon=True)

        ax.set_xlim(0.0, 1.0)
        # autoscale individual y-range; just add a small padding for clarity
    ...
    plt.subplots_adjust(bottom=0.15, left=0.03, right=0.99, top=0.90, wspace=0.1)
