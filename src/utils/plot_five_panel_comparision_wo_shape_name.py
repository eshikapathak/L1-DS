...
def robust_get_target_from_npz(path: Path) -> Optional[np.ndarray]:
    ...
    # Checking for 'rollout' as per user's robust_get_target_from_npz update
    for k in ["rollout"]:
        if k in d:
            arr = np.asarray(d[k])
...
def main():
    ...
    fig, axes = plt.subplots(1, 5, figsize=(19, 4.4), constrained_layout=False)
    letters = ["a","b","c","d","e"]
    ...
    GOAL_MARKER = '*'
    GOAL_MS = 14 # Marker Size
    GOAL_COLOR = COLORS["TARGET"]
    # -----------------------------------

    # === (a) LASA: Angle / no_llc_pulses ===
    shape = "Angle"
    ...
    # ADD GOAL MARKER
    ax.plot(ref_xy[-1, 0], ref_xy[-1, 1], marker=GOAL_MARKER, ms=GOAL_MS, color=GOAL_COLOR, zorder=5, clip_on=False) 
    ...
    # === (c) LASA: DoubleBendedLine / with_llc_matched_multisine ===
    ...
    target_xy = robust_get_target_from_npz(tgt_npz) #or logs["NODE"] # Use robust target, fallback to NODE log if needed
    ...
    # ADD GOAL MARKER
    # ax.plot(target_xy[-1, 0], target_xy[-1, 1], marker=GOAL_MARKER, ms=GOAL_MS, color=GOAL_COLOR, zorder=5, clip_on=False)
    ax.plot(logs["NODE"][:,0],     logs["NODE"][:,1],     lw=LW_NODE,    ls=LINE_STYLES["NODE"],    color=COLORS["NODE"])
    ax.plot(logs["NODE+CLF"][:,0], logs["NODE+CLF"][:,1], lw=LW_NODE_CLF, ls=LINE_STYLES["NODE+CLF"], color=COLORS["NODE+CLF"])
    ax.plot(logs["L1-NODE"][:,0],  logs["L1-NODE"][:,1],  lw=LW_L1_NODE, ls=LINE_STYLES["L1-NODE"], color=COLORS["L1-NODE"])
    ...
    handles = [
        # --- GOAL MARKER ADDED TO LEGEND ---
        plt.Line2D([0], [0], color=GOAL_COLOR, marker=GOAL_MARKER, ms=GOAL_MS, linestyle='None', label="Goal"),
        # -----------------------------------
    ...
    # Increased ncol to 5 for the new 'Goal' entry
