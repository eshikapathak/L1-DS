...
def _multisine_vec_factory(mag=1.0, K=7, fmin=0.2, fmax=2.0,
...
def _chirp_vec_factory(mag=1.0, f0=0.2, f1=2.0, T=8.0,
...
def _bandlimited_noise_vec_factory(mag=1.0, K=16, fmin=0.2, fmax=3.0,
...
# -------------------------------
# ---------- with_llc disturbances (enter through plant) ----------
def make_matched_fn(on=True, kind="sine", MAG=1.0, FREQ=0.5, **kw):
...
def make_unmatched_fn(on=True, kind="sine", MAG=1.0, FREQ=0.5, **kw):
    if not on: return constant_vec((0.0, 0.0))
