# src/experiments/run_experiments_periodic.py
...
def main():
    ...
    # width/depth are not stored; pass the same as training (128/3 default here)
    # The template only establishes shapes; eqx will overwrite with saved leaves.
    ...
    pos_rs, vel_rs, t_rs = resample(iros, nsamples=args.nsamples)
    ...
    for name, flags, sel_factory in controllers:
        ...
        npz_name = f"{args.shape}_{base_tag}_{name.replace('+','_')}.npz"
        logs = simulate(
    ...
    title = f"{args.shape} — {('LLC' if args.with_llc else 'No LLC')} — {dist_title}"
    fig_path = out_dir / f"fig_{args.shape}_{base_tag}.png"
    ...
    chart_path = out_dir / f"chart_{args.shape}_{base_tag}_dtw_avg.png"
